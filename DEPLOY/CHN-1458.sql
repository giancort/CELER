...
IF OBJECT_ID('SP_DATA_FAILURE_PAYMENTS') IS NOT NULL DROP PROCEDURE SP_DATA_FAILURE_PAYMENTS;
    GO
CREATE PROCEDURE SP_DATA_FAILURE_PAYMENTS(
...
BEGIN
    ...
    SELECT F.FILE_SEQUENCE,
        F.CREATED_AT FILE_DATE,
        F.AMOUNT,
        BDE.AGENCY,
        BDE.DIGIT_AGENCY,
        BDE.ACCOUNT,
        ...
        B.CODE       BANK_CODE,
        CE.COD_EC,
        CE.NAME      MERCHANT_NAME,
        CE.CPF_CNPJ  MERCHANT_DOC,
        AFF.NAME     AFF_NAME,
        AFF.COD_AFFILIATOR
    FROM FINANCE_SCHEDULE_FILE F
        JOIN @TP TP ON F.FILE_SEQUENCE = TP.CODE
        JOIN BANK_DETAILS_EC BDE on F.COD_BK_EC = BDE.COD_BK_EC
        JOIN ACCOUNT_TYPE A on BDE.COD_TYPE_ACCOUNT = A.COD_TYPE_ACCOUNT
        JOIN BANKS B on BDE.COD_BANK = B.COD_BANK
        JOIN COMMERCIAL_ESTABLISHMENT CE on BDE.COD_EC = CE.COD_EC
        JOIN AFFILIATOR AFF on CE.COD_AFFILIATOR = AFF.COD_AFFILIATOR
    WHERE F.TYPE_BANK = @TYPE_BANK
END
...
-- WebhookPagamentosEfetuados

IF NOT EXISTS
    (SELECT 1
FROM [SYS].[COLUMNS]
WHERE name = N'WEBHOOK_SENDED'
       AND object_id = OBJECT_ID(N'PROTOCOLS'))
    BEGIN

       alter table PROTOCOLS
            ADD
                WEBHOOK_SENDED BIT DEFAULT 0;
END;
GO

IF (SELECT COUNT(*)
FROM WEBHOOK_PROVIDER
WHERE NAME = 'payment_made_report')
    = 0
    insert into WEBHOOK_PROVIDER
       (name, description)
values
       ('payment_made_report', 'report of payments made')

    GO

IF OBJECT_ID('SP_LS_PROTOCOLS_TO_SEND') IS NOT NULL DROP PROCEDURE SP_LS_PROTOCOLS_TO_SEND;
GO
CREATE PROCEDURE SP_LS_PROTOCOLS_TO_SEND(@COD_AFF INT)
AS
BEGIN
    ...
    WITH CTE
             ...
             (
                 ...
                 from PROTOCOLS
                          JOIN COMMERCIAL_ESTABLISHMENT on COMMERCIAL_ESTABLISHMENT.COD_EC = PROTOCOLs.COD_EC
                          JOIN BANK_DETAILS_EC BDE ON PROTOCOLS.COD_BK_EC = BDE.COD_BK_EC
                          JOIN BANKS B on BDE.COD_BANK = B.COD_BANK
                          JOIN ACCOUNT_TYPE ON BDE.COD_TYPE_ACCOUNT = ACCOUNT_TYPE.COD_TYPE_ACCOUNT
                          JOIN ARRANG_TO_PAY ON PROTOCOLS.COD_PAY_PROT = ARRANG_TO_PAY.COD_PAY_PROT
                          JOIN DETAILS_ARRANG_TO_PAY DATP on ARRANG_TO_PAY.COD_ARR_PAY = DATP.COD_ARR_PAY
                          JOIN TRANSACTION_TITLES (NOLOCK) ON DATP.COD_TITTLE = TRANSACTION_TITLES.COD_TITLE
                          JOIN [TRANSACTION] (NOLOCK) ON TRANSACTION_TITLES.COD_TRAN = [TRANSACTION].COD_TRAN
                 where PROTOCOLS.WEBHOOK_SENDED = 0
                   AND COMMERCIAL_ESTABLISHMENT.COD_AFFILIATOR = @COD_AFF
                   and PROTOCOLS.CREATED_AT between dateadd(day, -1, getdate()) and getdate()
                 UNION ALL
                 select PROTOCOLS.COD_PAY_PROT,
                        PROTOCOLS.CREATED_AT                                         PAYMENT_DATE,
                        PROTOCOL,
                 ...
                 from PROTOCOLS
                          ...
                          JOIN ARRANG_TO_PAY ON PROTOCOLS.COD_PAY_PROT = ARRANG_TO_PAY.COD_PAY_PROT
                          JOIN DETAILS_ARRANG_TO_PAY DATP on ARRANG_TO_PAY.COD_ARR_PAY = DATP.COD_ARR_PAY
                          JOIN RELEASE_ADJUSTMENTS (NOLOCK) ON DATP.COD_REL_ADJ = RELEASE_ADJUSTMENTS.COD_REL_ADJ
                          JOIN TYPE_JUSTIFICATION TJ on RELEASE_ADJUSTMENTS.COD_TYPEJUST = TJ.COD_TYPEJUST
                 where PROTOCOLS.WEBHOOK_SENDED = 0
                   AND COMMERCIAL_ESTABLISHMENT.COD_AFFILIATOR = @COD_AFF
                   and PROTOCOLS.CREATED_AT between dateadd(day, -1, getdate()) and getdate()
                 UNION ALL
                 ...
                 from PROTOCOLS
                          ...
                          JOIN ARRANG_TO_PAY ON PROTOCOLS.COD_PAY_PROT = ARRANG_TO_PAY.COD_PAY_PROT
                          JOIN DETAILS_ARRANG_TO_PAY DATP on ARRANG_TO_PAY.COD_ARR_PAY = DATP.COD_ARR_PAY
                          JOIN TARIFF_EC (NOLOCK) ON DATP.COD_TARIFF_EC = TARIFF_EC.COD_TARIFF_EC
                          JOIN TYPE_TARIFF ON TARIFF_EC.COD_TTARIFF = TYPE_TARIFF.COD_TTARIFF
                 where PROTOCOLS.WEBHOOK_SENDED = 0
                   AND COMMERCIAL_ESTABLISHMENT.COD_AFFILIATOR = @COD_AFF
                   and PROTOCOLS.CREATED_AT between dateadd(day, -1, getdate()) and getdate()
                 UNION ALL
                 ...
                 from PROTOCOLS
                          ...
                          JOIN ARRANG_TO_PAY ON PROTOCOLS.COD_PAY_PROT = ARRANG_TO_PAY.COD_PAY_PROT
                          JOIN DETAILS_ARRANG_TO_PAY DATP on ARRANG_TO_PAY.COD_ARR_PAY = DATP.COD_ARR_PAY
                          JOIN FINANCIAL_BILLET (NOLOCK) ON DATP.COD_FIN_BILLET = FINANCIAL_BILLET.COD_FIN_BILLET
                          JOIN BILLET_TRANSACTION ON FINANCIAL_BILLET.COD_BILLET = BILLET_TRANSACTION.COD_BILLET
                 where PROTOCOLS.WEBHOOK_SENDED = 0
                   AND COMMERCIAL_ESTABLISHMENT.COD_AFFILIATOR = @COD_AFF
                   and PROTOCOLS.CREATED_AT between dateadd(day, -1, getdate()) and getdate()
             )
...
IF OBJECT_ID('SP_UP_PROTOCOLS_TO_SEND') IS NOT NULL DROP PROCEDURE SP_UP_PROTOCOLS_TO_SEND;
GO
create procedure SP_UP_PROTOCOLS_TO_SEND(
    @TP CODE_TYPE READONLY
)
AS
BEGIN
    ...
    UPDATE PROTOCOLS
    SET WEBHOOK_SENDED = 1
    FROM PROTOCOLS
             JOIN @TP TP ON TP.CODE = PROTOCOLS.COD_PAY_PROT
    WHERE PROTOCOLS.WEBHOOK_SENDED = 0;
...
SELECT
   ...
   ,[VW_REPORT_TRANSACTIONS_PRCS].EMAIL INTO [#TB_REPORT_TRANSACTIONS_PRCS_INSERT]
FROM [dbo].[VW_REPORT_TRANSACTIONS_PRCS]
WHERE [VW_REPORT_TRANSACTIONS_PRCS].[REP_COD_TRAN] IS NULL;
...
[EMAIL])
	SELECT
	   ...
	   ,[#TB_REPORT_TRANSACTIONS_PRCS_INSERT].[COD_EC_PROD]
	   ,[#TB_REPORT_TRANSACTIONS_PRCS_INSERT].TERMINAL_VERSION
	   ,[#TB_REPORT_TRANSACTIONS_PRCS_INSERT].[EMAIL]
	FROM [#TB_REPORT_TRANSACTIONS_PRCS_INSERT];
...
SET @QUERY_BASIS_PARAMETERS = CONCAT(@QUERY_BASIS_PARAMETERS, ' AND [REPORT_TRANSACTIONS].EMAIL = @EMAIL ');
...
(SELECT TOP (1000)
	...
	AND COALESCE([TRANSACTION_DATA_EXT].[NAME], '0') IN ('NSU', 'RCPTTXID', 'AUTO', '0')
	AND DATEADD(MINUTE, -5, GETDATE()) > [TRANSACTION].[CREATED_AT]
	AND [TRANSACTION].CODE = '342587540382472348')
...
SELECT
   ...
   ,[VW_REPORT_TRANSACTIONS_EXP].EMAIL INTO #TB_REPORT_TRANSACTIONS_EXP_INSERT
FROM [dbo].[VW_REPORT_TRANSACTIONS_EXP]
WHERE [VW_REPORT_TRANSACTIONS_EXP].[REP_COD_TRAN] IS NULL;
