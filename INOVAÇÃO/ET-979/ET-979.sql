...
IF OBJECT_ID('Recurring.RECURRING_PAYMENT') IS NOT NULL DROP TABLE Recurring.RECURRING_PAYMENT;
GO
...
IF OBJECT_ID('Recurring.CARD_TOKEN') IS NOT NULL DROP TABLE Recurring.CARD_TOKEN;
GO
...
IF OBJECT_ID('Recurring.ASS_RECURRING_TOKEN') IS NOT NULL DROP TABLE Recurring.ASS_RECURRING_TOKEN;
GO
...
IF OBJECT_ID('Recurring.DATE_RECURRING_PAYMENT') IS NOT NULL DROP TABLE Recurring.DATE_RECURRING_PAYMENT;
GO
...
IF OBJECT_ID('Recurring.SCHEDULE_DATA_RECURRING') IS NOT NULL DROP TABLE Recurring.SCHEDULE_DATA_RECURRING;
GO
...
IF OBJECT_ID('Recurring.TRANSACTION_RECURRING') IS NOT NULL DROP TABLE Recurring.TRANSACTION_RECURRING;
GO
...
IF OBJECT_ID('Recurring.CUSTOMER_RECURRING') IS NOT NULL DROP TABLE Recurring.CUSTOMER_RECURRING;
GO
...
IF OBJECT_ID('Recurring.LINK_RECURRING') IS NOT NULL DROP TABLE Recurring.LINK_RECURRING;
GO
...
IF OBJECT_ID('Recurring.DATE_RECURRING_LINK') IS NOT NULL DROP TABLE Recurring.DATE_RECURRING_LINK;
GO
...
IF OBJECT_ID('Recurring.RECURRING_SPLIT') IS NOT NULL DROP table Recurring.RECURRING_SPLIT;
GO
...
GO
IF OBJECT_ID('Recurring.SP_REG_RECURRING') IS NOT NULL DROP PROCEDURE Recurring.SP_REG_RECURRING;
GO
...
BEGIN
    ...
    SELECT @COD_CUSTOMER_REC = COD_CUSTOMER_REC
    ...
    IF @COD_CUSTOMER_REC IS NULL
        BEGIN
            ...
            VALUES (@CUSTOMER_NAME, @CUSTOMER_CPF_CNPJ, @CUSTOMER_BIRTHDATE,
                    @CUSTOMER_EMAIL, @CUSTOMER_PHONE, @CUSTOMER_ZIPCODE, @CUSTOMER_STREET, @CUSTOMER_NUMBER,
                    @CUSTOMER_COMPLEMENT, @CUSTOMER_NEIGH, @CUSTOMER_CITY, @CUSTOMER_UF);
    ...
    IF @LINK_CODE IS NOT NULL
        BEGIN
            SELECT NAME,
                   ...
                   HAS_EMAIL
            INTO #tmpLinkRecurring
            FROM Recurring.LINK_RECURRING
                     JOIN PAYMENT_LINK PL on LINK_RECURRING.COD_PAY_LINK = PL.COD_PAY_LINK AND PL.ACTIVE = 1 AND
                                             EXPIRATION_DATE >= dbo.FN_FUS_UTF(GETDATE())
            where pl.CODE = @LINK_CODE
            IF (SELECT COUNT(*) FROM #tmpLinkRecurring) <= 0
            ...
            FROM #tmpLinkRecurring tmp
                     JOIN COMMERCIAL_ESTABLISHMENT CE on tmp.COD_AFFILIATOR = CE.COD_AFFILIATOR AND
                                                         CE.CPF_CNPJ = tmp.DOCUMENT and ce.ACTIVE = 1
            SET @COD_REC_PAYMENT = @@IDENTITY;
            INSERT INTO Recurring.DATE_RECURRING_PAYMENT(COD_REC_PAYMENT, RECURRING_DATE)
            select @COD_REC_PAYMENT, INITIAL_DATE
            from #tmpLinkRecurring
        END
    ELSE
        BEGIN
            DECLARE @COD_EC INT = NULL;
            SELECT @COD_EC = COMMERCIAL_ESTABLISHMENT.COD_EC
            ...
            INSERT INTO Recurring.RECURRING_PAYMENT(NAME, DESCRIPTION, COD_EC, COD_SITUATION, AMOUNT, PERIOD_TYPE,
                                                    ...
                                                    FINAL_DATE, URL_CALLBACK, HAS_EMAIL, EMAIL, COD_CUSTOMER_REC,
                                                    COD_AFFILIATOR)
            VALUES (@NAME, @DESCRIPTION, @COD_EC, 32, @AMOUNT, @PERIOD_TYPE, @INITIAL_DATA, @FINAL_DATA, @URL_CALLBACK,
                    @HAS_EMAIL, @CUSTOMER_EMAIL, @COD_CUSTOMER_REC, @COD_AFF);
            SET @COD_REC_PAYMENT = @@IDENTITY;
            INSERT INTO Recurring.DATE_RECURRING_PAYMENT(COD_REC_PAYMENT, RECURRING_DATE)
            SELECT @COD_REC_PAYMENT, TP.INTERVAL
            FROM @TP_DATE TP
        END
    ...
    IF @IS_SPLIT = 1
        BEGIN
            INSERT INTO Recurring.RECURRING_SPLIT
            (COD_EC,
             COD_AFFILIATOR,
             AMOUNT,
             COD_REC_PAYMENT)
            SELECT COMMERCIAL_ESTABLISHMENT.COD_EC,
                   ...
                   @COD_REC_PAYMENT
            FROM COMMERCIAL_ESTABLISHMENT
                     JOIN @PROVIDERS TP ON TP.DOC_MERCHANT = COMMERCIAL_ESTABLISHMENT.CPF_CNPJ
    ...
    INSERT INTO Recurring.CARD_TOKEN (TOKEN, BRAND, COD_SITUATION, PAN) VALUES (@CARD_TOKEN, @BRAND, 32, @PAN);
    SET @COD_CARD_TOKEN = @@IDENTITY;
    IF @@ROWCOUNT < 1
        THROW 60000,'COULD NOT REGISTER [CARD_TOKEN] ',1;
    INSERT INTO ASS_RECURRING_TOKEN (COD_CARD_TOKEN, COD_REC_PAYMENT) values (@COD_CARD_TOKEN, @COD_REC_PAYMENT);
    IF @@ROWCOUNT < 1
        THROW 60000,'COULD NOT REGISTER [ASS_RECURRING_TOKEN] ',1;
    SELECT @COD_REC_PAYMENT;
END
go
go


IF OBJECT_ID('Recurring.SP_UP_RECURRING') IS NOT NULL DROP PROCEDURE Recurring.SP_UP_RECURRING;
GO
...
BEGIN
    ...
    SELECT @CARD_TOKEN = CT.COD_CARD_TOKEN
    FROM Recurring.ASS_RECURRING_TOKEN ART
             JOIN Recurring.CARD_TOKEN CT ON CT.COD_CARD_TOKEN = ART.COD_CARD_TOKEN
    ...
    SELECT @CARD_SITUATION = COD_SITUATION
    FROM CARD_TOKEN
    WHERE COD_CARD_TOKEN = @CARD_TOKEN;

    SELECT @RECURRING_SITUATION = COD_SITUATION
...
IF OBJECT_ID('Recurring.SP_REG_ASS_TRAN_RECURRING') IS NOT NULL DROP PROCEDURE Recurring.SP_REG_ASS_TRAN_RECURRING;
GO
...
begin
    ...
    select @service = COD_ITEM_SERVICE from ITEMS_SERVICES_AVAILABLE where NAME = 'RECURRING';
    
    SELECT @email = Recurring.RECURRING_PAYMENT.EMAIL,
    ...
    if @email IS NULL OR @NAME IS NULL
...
IF OBJECT_ID('SP_FD_EC_BY_DOC') IS NOT NULL DROP PROCEDURE SP_FD_EC_BY_DOC;
GO
...
BEGIN
    ...
    FROM COMMERCIAL_ESTABLISHMENT CE
             JOIN BRANCH_EC B ON B.COD_EC = CE.COD_EC
             JOIN DEPARTMENTS_BRANCH DB on B.COD_BRANCH = DB.COD_BRANCH
             LEFT JOIN ASS_DEPTO_EQUIP ADE on DB.COD_DEPTO_BRANCH = ADE.COD_DEPTO_BRANCH AND ADE.ACTIVE = 1
             LEFT JOIN EQUIPMENT E on ADE.COD_EQUIP = E.COD_EQUIP AND E.ACTIVE = 1 AND E.SERIAL = @SERIAL
...
IF OBJECT_ID('Recurring.SP_FD_DATA_RECURRING') IS NOT NULL DROP PROCEDURE Recurring.SP_FD_DATA_RECURRING;
GO
...
BEGIN
    ...
    from Recurring.RECURRING_PAYMENT RP
             ...
             JOIN TRADUCTION_SITUATION ON RP.COD_SITUATION = TRADUCTION_SITUATION.COD_SITUATION
    where COD_REC_PAYMENT = @COD_REC_PAYMENT AND RP.COD_AFFILIATOR = @COD_AFFILIATOR
END
go
...
AS
BEGIN
    SELECT [TRANSACTION].CODE           AS NSU,
           [TRANSACTION].BRAND,
           [TRANSACTION].BRAZILIAN_DATE AS TRANSACTION_DATE,
           TS.SITUATION_TR              AS SITUATION,
           TRANSACTION_RECURRING.INSTALLMENT
    FROM Recurring.RECURRING_PAYMENT RP
             JOIN Recurring.TRANSACTION_RECURRING on TRANSACTION_RECURRING.COD_REC_PAYMENT = RP.COD_REC_PAYMENT
             JOIN [TRANSACTION] WITH (NOLOCK) ON TRANSACTION_RECURRING.COD_TRAN = [TRANSACTION].COD_TRAN
             JOIN TRADUCTION_SITUATION TS on [TRANSACTION].COD_SITUATION = TS.COD_SITUATION
    WHERE RP.COD_REC_PAYMENT = @COD_REC_PAYMENT AND RP.COD_AFFILIATOR = @COD_AFFILIATOR

END
go


IF OBJECT_ID('Recurring.SCHEDULE_DATA_RECURRING') IS NULL BEGIN
    CREATE TABLE Recurring.SCHEDULE_DATA_RECURRING
    (
...
IF OBJECT_ID('Recurring.DATE_RECURRING_PAYMENT') IS NULL BEGIN
    CREATE TABLE Recurring.DATE_RECURRING_PAYMENT
    (
...
IF OBJECT_ID('Recurring.QUEUE') IS NULL BEGIN
...
GO
IF NOT EXISTS(select object_id from sys.indexes where name = 'IX_Recurring_DATES_TO_ENQUEUE')
    CREATE NONCLUSTERED INDEX IX_Recurring_DATES_TO_ENQUEUE ON  ReCurring.DATE_RECURRING_PAYMENT (ENQUEUED)

IF OBJECT_ID('Recurring.SP_LIST_RECURRING_PAYMENTS') IS NOT NULL BEGIN
...
AS BEGIN
    ...
    SELECT
        ...
        CT.BRAND,
        RP.COD_EC,
        IIF(EXISTS(SELECT COD_REC_PAYMENT FROM Recurring.RECURRING_SPLIT rs WHERE rs.COD_REC_PAYMENT = Q.COD_REC_PAYMENT), 1, 0) [HasSplit]
    INTO #toRun
    FROM Recurring.QUEUE Q
             JOIN Recurring.DATE_RECURRING_PAYMENT DRP ON Q.COD_DATE_REC_PAYMENT = DRP.COD_DATE_REC_PAYMENT
             JOIN Recurring.RECURRING_PAYMENT RP on DRP.COD_REC_PAYMENT = RP.COD_REC_PAYMENT AND RP.ACTIVE = 1
             JOIN Recurring.CUSTOMER_RECURRING CR ON CR.COD_CUSTOMER_REC = RP.COD_CUSTOMER_REC
             JOIN Recurring.ASS_RECURRING_TOKEN ART ON ART.COD_REC_PAYMENT = DRP.COD_REC_PAYMENT
             JOIN Recurring.CARD_TOKEN CT on ART.COD_CARD_TOKEN = CT.COD_CARD_TOKEN
    ...
    SELECT
        ...
        E.SERIAL [EQP_SERIAL],
        E.COD_EQUIP,
        ROW_NUMBER() over (PARTITION BY CE.COD_EC ORDER BY E.COD_EQUIP) Seq_Number
    INTO #Eqps
    FROM COMMERCIAL_ESTABLISHMENT CE
             JOIN BRANCH_EC BE ON CE.COD_EC = BE.COD_EC
             JOIN DEPARTMENTS_BRANCH DB ON DB.COD_BRANCH = BE.COD_BRANCH
             JOIN ASS_DEPTO_EQUIP ADE on DB.COD_DEPTO_BRANCH = ADE.COD_DEPTO_BRANCH AND BE.ACTIVE = 1
             JOIN EQUIPMENT E on ADE.COD_EQUIP = E.COD_EQUIP AND E.COD_MODEL = 6 AND E.ACTIVE = 1
    WHERE CE.COD_EC IN (SELECT COD_EC FROM #Ecs) AND CE.ACTIVE = 1

    DELETE FROM #Eqps WHERE Seq_Number > 1

    SELECT tr.*, E2.*
    FROM #toRun tR
             JOIN #Eqps E2 on tR.COD_EC = E2.Ec_Id

END
GO

IF OBJECT_ID('Recurring.SP_START_PAYMENT_PROCESSING') IS NOT NULL BEGIN
...
AS BEGIN
    ...
    BEGIN TRY
        ...
        SELECT @STARTED_PROCESSING = STARTED_PROCESSING
        FROM Recurring.QUEUE
        WHERE COD_QUEUE = @COD_QUEUE;

        IF @STARTED_PROCESSING IS NULL OR DATEDIFF(minute, @STARTED_PROCESSING, GETDATE()) > 5 BEGIN
            UPDATE Recurring.QUEUE SET STARTED_PROCESSING = GETDATE()
            WHERE COD_QUEUE = @COD_QUEUE;
    ...
    BEGIN CATCH
        ROLLBACK TRANSACTION;
        SELECT 'FAILED TO LOCK QUEUE ITEM' [MESSAGE], ERROR_MESSAGE() [ErrorMessage], ERROR_NUMBER() AS ErrorNumber;
        RETURN;
    END CATCH
    ...
    SET @Recurring_Active = IIF ( 
        EXISTS(SELECT Q.COD_QUEUE 
                FROM RECURRING_PAYMENT RP 
                    JOIN QUEUE Q on RP.COD_REC_PAYMENT = Q.COD_REC_PAYMENT 
                WHERE Q.COD_QUEUE = @COD_QUEUE AND COD_SITUATION = 3), 1, 0 )
    
    IF @Recurring_Active = 0 BEGIN
        SELECT 'DELETED' [MESSAGE]
    END 
    ELSE  IF @STARTED_PROCESSING IS NULL BEGIN
        SELECT 'SUCCESS' [MESSAGE];
    END
    ELSE BEGIN
        DECLARE @ENDED_PROCESSING DATETIME

        SELECT @ENDED_PROCESSING = ENDED_PROCESSING
        FROM Recurring.QUEUE
        WHERE COD_QUEUE = @COD_QUEUE

        IF @ENDED_PROCESSING IS NULL
            SELECT 'STARTED' [MESSAGE];
        ELSE
            SELECT 'ENDED' [MESSAGE];

    END
END
GO
GO
IF OBJECT_ID('Recurring.SP_END_PAYMENT_PROCESSING') IS NOT NULL BEGIN 
...
AS BEGIN
    ...
    select @recurring = COD_ITEM_SERVICE from ITEMS_SERVICES_AVAILABLE where NAME = 'RECURRING';
    ...
    BEGIN TRY
        ...
        IF @ENDED_PROCESSING IS NULL OR DATEDIFF(minute, @ENDED_PROCESSING, GETDATE()) > 5 BEGIN
            UPDATE Recurring.QUEUE SET ENDED_PROCESSING = GETDATE()
            WHERE COD_DATE_REC_PAYMENT = @COD_DATE_REC_PAYMENT;
    ...
    IF NOT EXISTS(SELECT COD_REC_PAYMENT FROM Recurring.DATE_RECURRING_PAYMENT WHERE COD_DATE_REC_PAYMENT = @COD_DATE_REC_PAYMENT AND COD_REC_PAYMENT = @COD_REC_PAYMENT)
        THROW 70012, 'INVALID RECURRING PAYMENT CODE', 1;
    ...
    SET @Recurring_Active = IIF (
            EXISTS(SELECT Q.COD_QUEUE
                   FROM RECURRING_PAYMENT RP
                            JOIN QUEUE Q on RP.COD_REC_PAYMENT = Q.COD_REC_PAYMENT
                   WHERE Q.COD_REC_PAYMENT = @COD_REC_PAYMENT AND COD_SITUATION = 3), 1, 0 )

    IF @Recurring_Active = 1 AND @NSU IS NOT NULL BEGIN
        DECLARE @COD_TRAN INT = NULL

        SELECT @COD_TRAN = COD_TRAN FROM [TRANSACTION] WITH (NOLOCK) WHERE CODE = @NSU
        ...
        SELECT @COD_TRAN_DPL = @COD_TRAN FROM Recurring.TRANSACTION_RECURRING WITH (NOLOCK) WHERE COD_REC_PAYMENT = @COD_REC_PAYMENT AND [INSTALLMENT] = @Sequence

        IF @COD_TRAN_DPL IS NOT NULL AND @COD_TRAN_DPL != @COD_TRAN
            THROW 70012, 'POSSIBLE DOUBLE CHARGE', 1

        IF @COD_TRAN_DPL IS NULL BEGIN
            INSERT INTO Recurring.TRANSACTION_RECURRING (COD_TRAN, COD_REC_PAYMENT, INSTALLMENT) VALUES (@COD_TRAN, @COD_REC_PAYMENT, @Sequence);

            INSERT INTO [TRANSACTION_SERVICES] (COD_ITEM_SERVICE, COD_TRAN) VALUES (@recurring, @COD_TRAN)

            UPDATE [TRANSACTION]
            SET [CUSTOMER_EMAIL] = @Email, [CUSTOMER_IDENTIFICATION] = @Name, [DESCRIPTION] = @TranDescription, [TRACKING_TRANSACTION] = @COD_REC_PAYMENT
            WHERE COD_TRAN = @COD_TRAN
        END
    END

    IF @Recurring_Active = 1 AND @ENDED_PROCESSING IS NULL BEGIN

        IF @MailJsonData IS NOT NULL BEGIN
            INSERT INTO Recurring.QUEUE_MAIL (RETRY_COUNT, COD_REC_PAYMENT, COD_DATE_REC_PAYMENT, RUN_AT, JSON_DATA)
            SELECT 0, COD_REC_PAYMENT, COD_DATE_REC_PAYMENT, GETDATE(), @MailJsonData FROM Recurring.QUEUE
            WHERE COD_DATE_REC_PAYMENT = @COD_DATE_REC_PAYMENT;
        END

        IF @NotifyJsonData IS NOT NULL BEGIN
            INSERT INTO Recurring.QUEUE_NOTIFICATION (RETRY_COUNT, COD_REC_PAYMENT, COD_DATE_REC_PAYMENT, RUN_AT, JSON_DATA)
            SELECT 0, COD_REC_PAYMENT, COD_DATE_REC_PAYMENT, GETDATE(), @NotifyJsonData FROM Recurring.QUEUE
            WHERE COD_DATE_REC_PAYMENT = @COD_DATE_REC_PAYMENT;
        END
        ...
        SELECT COD_DATE_REC_PAYMENT, COD_REC_PAYMENT, 3, RETRY_COUNT, RUN_AT, 'SUCCESS' FROM Recurring.QUEUE
        WHERE COD_DATE_REC_PAYMENT = @COD_DATE_REC_PAYMENT;
        ...
        IF @FinalDate IS NULL OR (@NextExecutionDate <= @FinalDate and @FinalDate is not null)
            BEGIN
                INSERT INTO Recurring.DATE_RECURRING_PAYMENT (COD_REC_PAYMENT, RECURRING_DATE, ENQUEUED, SEQUENCE)
                SELECT COD_REC_PAYMENT, @NextExecutionDate, 0, (@Sequence + 1)
        ...
        SELECT 'SUCCESS' [MESSAGE];
    END
    ELSE IF @Recurring_Active = 0 BEGIN
        SELECT 'DELETED' [MESSAGE]
    END
    ELSE BEGIN
...
IF OBJECT_ID('Recurring.QUEUE_MAIL') IS NULL BEGIN
...
IF OBJECT_ID('Recurring.QUEUE_NOTIFICATION') IS NULL BEGIN
...
AS BEGIN
    ...
    IF @NSU IS NOT NULL BEGIN
        ...
        IF @COD_TRAN_DPL IS NULL BEGIN
            ...
            INSERT INTO [TRANSACTION_SERVICES] (COD_ITEM_SERVICE, COD_TRAN) VALUES (17, @COD_TRAN)
    ...
    IF @ENDED_PROCESSING IS NULL BEGIN
        ...
        IF @FinalDate IS NOT NULL AND @NextExecutionDate < @FinalDate BEGIN
            INSERT INTO Recurring.DATE_RECURRING_PAYMENT (COD_REC_PAYMENT, RECURRING_DATE, ENQUEUED, SEQUENCE)
            SELECT COD_REC_PAYMENT, @NextExecutionDate, 0, (@Sequence + 1)
            FROM Recurring.DATE_RECURRING_PAYMENT
            WHERE COD_DATE_REC_PAYMENT = @COD_DATE_REC_PAYMENT
...
IF OBJECT_ID('Recurring.SP_RETRY_PAYMENT_PROCESSING') IS NOT NULL BEGIN
...
AS BEGIN
    ...
    IF EXISTS(SELECT COD_QUEUE FROM Recurring.QUEUE WHERE COD_QUEUE = @COD_QUEUE AND RUN_AT > @RAN_AT) BEGIN
        RETURN;
    END

    INSERT INTO Recurring.SCHEDULE_DATA_RECURRING (COD_DATE_REC_PAYMENT, COD_REC_PAYMENT, COD_SITUATION, RETRY_COUNT, RAN_AT, MESSAGE)
    SELECT COD_DATE_REC_PAYMENT, COD_REC_PAYMENT, 7, RETRY_COUNT, RUN_AT, @REASON
    FROM Recurring.QUEUE
    WHERE COD_QUEUE = @COD_QUEUE
...
IF OBJECT_ID('Recurring.SP_START_MAIL_SENDING') IS NOT NULL BEGIN
...
AS BEGIN
    ...
    BEGIN TRY 
        ...
        SELECT @START_PROCESSING = STARTED_PROCESSING
        FROM Recurring.QUEUE_MAIL
        WHERE COD_DATE_REC_PAYMENT = @COD_DATE_REC_PAYMENT
        
        IF @START_PROCESSING IS NULL OR DATEDIFF(MINUTE, @START_PROCESSING, GETDATE()) > 5 BEGIN 
            UPDATE Recurring.QUEUE_MAIL SET STARTED_PROCESSING = GETDATE()
            WHERE COD_DATE_REC_PAYMENT = @COD_DATE_REC_PAYMENT;
...
IF OBJECT_ID('Recurring.SP_END_MAIL_SENDING') IS NOT NULL BEGIN
...
AS BEGIN
    ...
    INSERT INTO Recurring.SCHEDULE_DATA_RECURRING (COD_DATE_REC_PAYMENT, COD_REC_PAYMENT, COD_SITUATION, RETRY_COUNT, RAN_AT, MESSAGE)
    SELECT COD_DATE_REC_PAYMENT, COD_REC_PAYMENT, 3, RETRY_COUNT, dbo.FN_FUS_UTF(GETDATE()), 'EMAIL ENVIADO' 
    FROM Recurring.QUEUE_MAIL
    WHERE COD_DATE_REC_PAYMENT = @COD_DATE_REC_PAYMENT
...
IF OBJECT_ID('Recurring.SP_START_PAYMENT_NOTIFICATION') IS NOT NULL BEGIN
...
AS BEGIN
    ...
    BEGIN TRY
        ...
        IF @START_PROCESSING IS NULL OR DATEDIFF(MINUTE, @START_PROCESSING, GETDATE()) > 5 BEGIN
            UPDATE Recurring.QUEUE_NOTIFICATION SET STARTED_PROCESSING = GETDATE()
            WHERE COD_DATE_REC_PAYMENT = @COD_DATE_REC_PAYMENT;
...
IF OBJECT_ID('Recurring.SP_END_PAYMENT_NOTIFICATION') IS NOT NULL BEGIN
...
AS BEGIN
    ...
    INSERT INTO Recurring.SCHEDULE_DATA_RECURRING (COD_DATE_REC_PAYMENT, COD_REC_PAYMENT, COD_SITUATION, RETRY_COUNT, RAN_AT, MESSAGE)
    SELECT COD_DATE_REC_PAYMENT, COD_REC_PAYMENT, 3, RETRY_COUNT, dbo.FN_FUS_UTF(GETDATE()), 'NOTIFICACAO ENVIADA'
    FROM Recurring.QUEUE_NOTIFICATION
    WHERE COD_DATE_REC_PAYMENT = @COD_DATE_REC_PAYMENT
...
IF OBJECT_ID('Recurring.SP_RETRY_MAIL') IS NOT NULL BEGIN
...
AS BEGIN
    ...
    FROM Recurring.QUEUE_MAIL Q
             ...
             JOIN Recurring.CUSTOMER_RECURRING CR ON CR.COD_CUSTOMER_REC = RP.COD_CUSTOMER_REC
             JOIN COMMERCIAL_ESTABLISHMENT CE on RP.COD_EC = CE.COD_EC
    WHERE Q.RUN_AT <= @CURRENT_DATE AND DATEDIFF(MINUTE, Q.CREATE_AT, @CURRENT_DATE) > 5

END
GO

IF OBJECT_ID('Recurring.SP_RETRY_NOTIFY') IS NOT NULL BEGIN
...
IF OBJECT_ID('Recurring.FN_NEXT_RECURRING_DATE') IS NOT NULL BEGIN
...
IF OBJECT_ID('Recurring.SP_ADD_NEW_PAYMENT') IS NOT NULL BEGIN
...
AS BEGIN
    ...
    SELECT DRP.COD_REC_PAYMENT, 
           DRP.COD_DATE_REC_PAYMENT, 
           DRP.RECURRING_DATE, 
           RP.PERIOD_TYPE, 
           RP.FINAL_DATE, 
           DRP.SEQUENCE,
           Recurring.FN_NEXT_RECURRING_DATE(DRP.RECURRING_DATE, RP.PERIOD_TYPE) NEXT_DATE
    INTO #toProcess
    FROM ReCurring.DATE_RECURRING_PAYMENT DRP
             JOIN Recurring.RECURRING_PAYMENT RP ON RP.COD_REC_PAYMENT = DRP.COD_REC_PAYMENT AND RP.ACTIVE = 1 AND RP.COD_SITUATION != 7
             JOIN Recurring.CUSTOMER_RECURRING CR ON CR.COD_CUSTOMER_REC = RP.COD_CUSTOMER_REC
             JOIN Recurring.ASS_RECURRING_TOKEN ART ON ART.COD_REC_PAYMENT = DRP.COD_REC_PAYMENT
             JOIN Recurring.CARD_TOKEN CT on ART.COD_CARD_TOKEN = CT.COD_CARD_TOKEN
    WHERE DRP.ENQUEUED = 0 AND RP.COD_REC_PAYMENT = @COD_REC_PAYMENT
    
    INSERT INTO Recurring.DATE_RECURRING_PAYMENT (COD_REC_PAYMENT, RECURRING_DATE, SEQUENCE)
    SELECT tp.COD_REC_PAYMENT, tp.NEXT_DATE, (tp.SEQUENCE + 1 )
    FROM #toProcess tp
    WHERE tp.SEQUENCE = 1 AND (tp.FINAL_DATE IS NULL OR tp.[FINAL_DATE] >= tp.NEXT_DATE)

    UPDATE DRP
    SET ENQUEUED = 1
    FROM Recurring.DATE_RECURRING_PAYMENT DRP
    JOIN #toProcess t on DRP.COD_DATE_REC_PAYMENT = t.COD_DATE_REC_PAYMENT AND t.SEQUENCE = 1
    
    SELECT DISTINCT DRP.COD_REC_PAYMENT, DRP.COD_DATE_REC_PAYMENT, DRP.RECURRING_DATE
    INTO #toInsert
    FROM Recurring.DATE_RECURRING_PAYMENT DRP
    JOIN #toProcess tP on DRP.COD_REC_PAYMENT = tP.COD_REC_PAYMENT
    WHERE DRP.ENQUEUED = 0
    
    INSERT INTO Recurring.QUEUE(COD_REC_PAYMENT, COD_DATE_REC_PAYMENT, RUN_AT)
    SELECT p.COD_REC_PAYMENT, p.COD_DATE_REC_PAYMENT, p.RECURRING_DATE 
    FROM #toInsert p
            LEFT JOIN Recurring.QUEUE Q ON p.COD_DATE_REC_PAYMENT = Q.COD_DATE_REC_PAYMENT
    WHERE Q.COD_QUEUE IS NULL

    UPDATE DRP
    SET ENQUEUED = 1
    FROM ReCurring.DATE_RECURRING_PAYMENT DRP
            JOIN #toInsert p on p.COD_DATE_REC_PAYMENT = DRP.COD_DATE_REC_PAYMENT
END
GO
IF OBJECT_ID('Recurring.SP_RM_PAYMENT') IS NOT NULL BEGIN
...
AS BEGIN
    ...
    DELETE FROM Recurring.QUEUE   
    WHERE COD_REC_PAYMENT = @COD_REC_PAYMENT AND
          STARTED_PROCESSING IS NULL OR 
          (ENDED_PROCESSING IS NULL AND DATEDIFF(MINUTE, STARTED_PROCESSING, @TODAY) > 5)
...
END
GO
IF OBJECT_ID('Recurring.SP_LIST_SPLIT_INFO') IS NOT NULL BEGIN
...
AS BEGIN
    ...
    SELECT COD_REC_SPLIT, RS.COD_EC, CE.CPF_CNPJ [EC_DOC], RS.COD_AFFILIATOR, A.CPF_CNPJ [AFF_DOC], AMOUNT 
    FROM Recurring.RECURRING_SPLIT RS WITH (NOLOCK)
    JOIN AFFILIATOR A WITH (NOLOCK) on RS.COD_AFFILIATOR = A.COD_AFFILIATOR
    JOIN COMMERCIAL_ESTABLISHMENT CE WITH (NOLOCK) ON RS.COD_EC = CE.COD_EC
    WHERE COD_REC_PAYMENT = @COD_REC_PAYMENT
    
END
GO

IF OBJECT_ID('Recurring.SP_GIVE_UP_PAYMENT') IS NOT NULL BEGIN
...
(
    ...
    IF @ENDED_PROCESSING IS NULL BEGIN
        ...
        INSERT INTO Recurring.SCHEDULE_DATA_RECURRING (COD_DATE_REC_PAYMENT, COD_REC_PAYMENT, COD_SITUATION, RETRY_COUNT, RAN_AT, MESSAGE)
        SELECT COD_DATE_REC_PAYMENT, COD_REC_PAYMENT, 7, RETRY_COUNT, RUN_AT, @Reason FROM Recurring.QUEUE
        WHERE COD_DATE_REC_PAYMENT = @COD_DATE_REC_PAYMENT;
        ...
        FROM Recurring.ASS_RECURRING_TOKEN ACT
                 JOIN Recurring.CARD_TOKEN C ON C.COD_CARD_TOKEN = ACT.COD_CARD_TOKEN
...
IF OBJECT_ID('Recurring.SP_REMOVE_QUEUE_ITEM') IS NOT NULL BEGIN
...
END
GO
IF OBJECT_ID('Recurring.SP_UP_RECURRING') IS NOT NULL BEGIN
...
IF OBJECT_ID('Recurring.SP_REPORT_PLANS') IS NOT NULL BEGIN
...
AS BEGIN
    ...
    DECLARE @SQL NVARCHAR(MAX), @OFFSET INT = ((@PAGE - 1) * @PAGE_SIZE)
    ...
    IF EXISTS(SELECT CODE FROM @COD_SITUATION) BEGIN
        IF EXISTS(SELECT CODE FROM @COD_SITUATION WHERE CODE = 'active')
            INSERT INTO @IdsSituation VALUES (3)
        
        IF EXISTS(SELECT CODE FROM @COD_SITUATION WHERE CODE = 'canceled')
            INSERT INTO @IdsSituation VALUES (6)
    
        IF EXISTS(SELECT CODE FROM @COD_SITUATION WHERE CODE = 'expired') BEGIN
...
IF OBJECT_ID('Recurring.SP_REPORT_STAT_PLANS') IS NOT NULL BEGIN
...
IF OBJECT_ID('Recurring.SP_DETAIL_PLAN') IS NOT NULL BEGIN
...
AS BEGIN
    ...
    FROM Recurring.RECURRING_PAYMENT (NOLOCK) RP
        JOIN Recurring.CUSTOMER_RECURRING (NOLOCK) CR ON CR.COD_CUSTOMER_REC = RP.COD_CUSTOMER_REC
        JOIN COMMERCIAL_ESTABLISHMENT (NOLOCK) CE ON RP.COD_EC = CE.COD_EC
...
AS BEGIN
    ...
    SELECT COD_REC_SPLIT, RS.COD_EC, CE.CPF_CNPJ [EC_DOC], RS.COD_AFFILIATOR, A.CPF_CNPJ [AFF_DOC], AMOUNT, CE.TRADING_NAME [EC_NAME], A.NAME [AFF_NAME], CE.TRADING_NAME [NAME_EC]
    FROM Recurring.RECURRING_SPLIT RS WITH (NOLOCK)
             JOIN AFFILIATOR A WITH (NOLOCK) on RS.COD_AFFILIATOR = A.COD_AFFILIATOR
             JOIN COMMERCIAL_ESTABLISHMENT CE WITH (NOLOCK) ON RS.COD_EC = CE.COD_EC
    WHERE COD_REC_PAYMENT = @COD_REC_PAYMENT
...
IF OBJECT_ID('Recurring.SP_PAYMENT_HISTORY') IS NOT NULL BEGIN
...
AS BEGIN
    ...
    SELECT DRP.COD_REC_PAYMENT,
       ...
       DRP.SEQUENCE,
       NULL  COD_SITUATION,
       (CASE WHEN TR.COD_TRAN_REC IS NOT NULL THEN 'success' WHEN NOT EXISTS(SELECT TOP 1 COD_SCHED_DATA_REC FROM Recurring.SCHEDULE_DATA_RECURRING (NOLOCK) SDR WHERE DRP.COD_DATE_REC_PAYMENT = SDR.COD_DATE_REC_PAYMENT) THEN 'ready' ELSE 'failed' END ) [LAST_SITUATION],
       (SELECT top 1 RAN_AT FROM Recurring.SCHEDULE_DATA_RECURRING (NOLOCK) SDR WHERE DRP.COD_DATE_REC_PAYMENT = SDR.COD_DATE_REC_PAYMENT and MESSAGE like 'SUCCESS'),
       Q.RETRY_COUNT,
       IIF(T.AMOUNT IS NULL, RP.AMOUNT, T.AMOUNT) [Amount],
       T.CODE [Nsu]
FROM Recurring.DATE_RECURRING_PAYMENT (NOLOCK) DRP
         JOIN Recurring.RECURRING_PAYMENT RP ON RP.COD_REC_PAYMENT = DRP.COD_REC_PAYMENT
         LEFT JOIN Recurring.QUEUE (NOLOCK) Q ON DRP.COD_DATE_REC_PAYMENT = Q.COD_DATE_REC_PAYMENT
         LEFT JOIN Recurring.TRANSACTION_RECURRING (NOLOCK) TR ON TR.COD_REC_PAYMENT = DRP.COD_REC_PAYMENT AND TR.INSTALLMENT = DRP.SEQUENCE
         LEFT JOIN [TRANSACTION] (NOLOCK) T ON T.COD_TRAN = TR.COD_TRAN          
    WHERE DRP.COD_REC_PAYMENT = @COD_REC_PAYMENT

END
GO

IF OBJECT_ID('Recurring.SP_EXPORT_PLANS') IS NOT NULL BEGIN
...
UPDATE PAYMENT_LINK SET IS_RECURRING = 0 WHERE IS_RECURRING IS NULL
...
BEGIN
    ...
    FROM PAYMENT_LINK
             JOIN COMMERCIAL_ESTABLISHMENT ON COMMERCIAL_ESTABLISHMENT.COD_EC = PAYMENT_LINK.COD_EC
             JOIN AFFILIATOR ON AFFILIATOR.COD_AFFILIATOR = PAYMENT_LINK.COD_AFFILIATOR
             JOIN THEMES ON THEMES.COD_AFFILIATOR = AFFILIATOR.COD_AFFILIATOR
        AND THEMES.ACTIVE = 1
...
begin
    ...
    select lr.NAME,
           ...
           lr.AMOUNT
    from PAYMENT_LINK
             join Recurring.LINK_RECURRING lr on lr.COD_PAY_LINK = PAYMENT_LINK.COD_PAY_LINK
    where CODE = @CODE
      and ACTIVE = 1
           ...
           (SELECT TOP 1 SERIAL
            FROM DEPARTMENTS_BRANCH DB
                     JOIN ASS_DEPTO_EQUIP ADE
                          on ade.COD_DEPTO_BRANCH = db.COD_DEPTO_BRANCH AND ADE.ACTIVE = 1
                     JOIN EQUIPMENT E on e.COD_EQUIP = ade.COD_EQUIP AND E.ACTIVE = 1 AND COD_MODEL = 6
...
GO
create procedure Recurring.SP_FD_DATA_EMAIL_RECURRING(
    @COD_REC_PAYMENT INT
)
as
begin
 select COMMERCIAL_ESTABLISHMENT.NAME as EC_NAME,
           rp.NAME                       as REC_NAME,
           rp.DESCRIPTION,
           rp.AMOUNT,
           rp.PERIOD_TYPE,
           ...
           THEMES.LOGO_HEADER_AFFILIATE
    from Recurring.RECURRING_PAYMENT rp
             left join AFFILIATOR on AFFILIATOR.COD_AFFILIATOR = rp.COD_AFFILIATOR
             left join THEMES on THEMES.COD_AFFILIATOR = AFFILIATOR.COD_AFFILIATOR and THEMES.ACTIVE = 1
             join COMMERCIAL_ESTABLISHMENT on COMMERCIAL_ESTABLISHMENT.COD_EC = rp.COD_EC
    where rp.COD_REC_PAYMENT = @COD_REC_PAYMENT
...
GO
create procedure Recurring.SP_FD_RECURRING_CUSTOMER(
    @COD_REC int
)
as
begin
    select rp.NAME as REC_NAME,
           rp.DESCRIPTION,
           rp.AMOUNT,
           rp.PERIOD_TYPE,
           rp.INITIAL_DATE,
    ...
    where rp.COD_REC_PAYMENT = @COD_REC and rp.COD_SITUATION = 3
end
go
...
BEGIN
    ...
    SELECT COD_REC_SPLIT, RS.COD_EC, CE.CPF_CNPJ [EC_DOC], RS.COD_AFFILIATOR, A.CPF_CNPJ [AFF_DOC], AMOUNT, CE.NAME [NAME_EC]
    FROM Recurring.RECURRING_SPLIT RS WITH (NOLOCK)
             JOIN AFFILIATOR A WITH (NOLOCK) on RS.COD_AFFILIATOR = A.COD_AFFILIATOR
             JOIN COMMERCIAL_ESTABLISHMENT CE WITH (NOLOCK) ON RS.COD_EC = CE.COD_EC
    WHERE COD_REC_PAYMENT = @COD_REC_PAYMENT
...
GO
CREATE procedure Recurring.SP_FD_DATA_EMAIL_BY_LINK(
    @CODE VARCHAR(255)
)
as
begin
    ...
    from PAYMENT_LINK PL
             join Recurring.LINK_RECURRING lr on lr.COD_PAY_LINK = pl.COD_PAY_LINK
             join COMMERCIAL_ESTABLISHMENT on COMMERCIAL_ESTABLISHMENT.COD_EC = pl.COD_EC
             join AFFILIATOR on AFFILIATOR.COD_AFFILIATOR = pl.COD_AFFILIATOR
             left join THEMES on THEMES.COD_AFFILIATOR = AFFILIATOR.COD_AFFILIATOR and THEMES.ACTIVE = 1
    where pl.CODE = @CODE
...
------------------------------------------------------------------------------------------    
    Luiz Aquino      V1         2020-08-27       Created   
    Lucas Aguiar     V2         2020-09-19       alterações hmg   
------------------------------------------------------------------------------------------*/
(@DATE_FROM DATETIME,
 @DATE_UNTIL DATETIME,
 @PLAN_NAME VARCHAR(128) = NULL,
...
BEGIN
    ...
    IF EXISTS(SELECT CODE FROM @COD_SITUATION)
        BEGIN
            IF EXISTS(SELECT CODE FROM @COD_SITUATION WHERE CODE = 'active')
                INSERT INTO @IdsSituation VALUES (3)
            ...
            IF EXISTS(SELECT CODE FROM @COD_SITUATION WHERE CODE = 'expired')
                BEGIN
                    SET @SQL += ' AND (RP.COD_SITUATION IN (SELECT CODE FROM @COD_SITUATION) OR (RP.COD_SITUATION = 3 AND RP.FINAL_DATE < @TODAY ) )'
                END
            ELSE
...
ALTER PROCEDURE [DBO].[SP_FIN_CALENDAR_TITLES_PRC](@COD_BK_EC INT,
                                                    @DATE DATE,
...
BEGIN
    ...
    SELECT [T].[CODE],
           ...
           CAST((([TT].[AMOUNT] * (1 - ([TT].[TAX_INITIAL] / 100)) * CASE
                                                                         WHEN [TT].[ANTICIP_PERCENT] IS NULL
                                                                             THEN 1
    ...
    FROM [FINANCE_CALENDAR] AS [F]
             JOIN [TRANSACTION_TITLES] AS [TT] WITH (NOLOCK) ON [TT].[COD_FIN_CALENDAR] = [F].[COD_FIN_CALENDAR]
             JOIN [TRANSACTION] AS [T] WITH (NOLOCK) ON [T].[COD_TRAN] = [TT].[COD_TRAN]
             JOIN [SITUATION] AS [S] ON [S].[COD_SITUATION] = [F].[COD_SITUATION]
             LEFT JOIN [FINANCE_SCHEDULE_FILE] ON [FINANCE_SCHEDULE_FILE].[COD_FIN_SCH_FILE] = [F].[COD_FIN_SCH_FILE]
             LEFT JOIN TRANSACTION_SERVICES TS WITH (NOLOCK) on TS.COD_TRAN =  T.COD_TRAN AND TS.COD_ITEM_SERVICE = @recurring
    WHERE [F].[ACTIVE] = 1
      AND [F].COD_EC = @COD_EC
...
alter PROCEDURE [SP_FINANCE_CALENDAR_PRC](@INITIAL_DATE DATETIME,
                                           @FINAL_DATE DATETIME,
...
ALTER VIEW [dbo].[VW_REPORT_FULL_CASH_FLOW]
    ...
    Caike Uchoa                       V7         04/09/2020               Add correção qtd_days quando spot ou debito
    Caike Uchoa                       v8         18/09/2020           Add Correção tarifa aff e Antecip_aff quando for dzero
    ---------------------------------------------------------------------------------------*/
AS
WITH CTE
         AS
         (SELECT --TOP(1000)                                                               
              TRANSACTION_TITLES.TAX_INITIAL
               ...
               , (CASE
                      WHEN (SELECT TRANSACTION_SERVICES.TAX_PLANDZERO_EC
                          ...
                          > 0 THEN dbo.FNC_CALC_DZERO_NET_VALUE_CONSOLIDATED(TRANSACTION_TITLES.AMOUNT,
                                                                             TRANSACTION_TITLES.PLOT,
                                                                             TRANSACTION_TITLES.TAX_INITIAL,
               ...
               , dbo.[FNC_ANT_VALUE_LIQ_DAYS](
                     ...
                     (CASE
                          WHEN TRANSACTION_TITLES.IS_SPOT = 1 THEN DATEDIFF(DAY, TRANSACTION_TITLES.PREVISION_PAY_DATE,
                                                                            TRANSACTION_TITLES.ORIGINAL_RECEIVE_DATE)
                     ...
                     dbo.[FNC_ANT_VALUE_LIQ_DAYS]
                         (
                             ...
                             COALESCE([TRANSACTION_TITLES_COST].[PERCENTAGE],
                                      TRANSACTION_TITLES.TAX_INITIAL) +
                             (CASE
                                  WHEN [TRANSACTION].COD_TTYPE = 2 THEN ISNULL(
                             ...
                             (CASE
                                  WHEN [TRANSACTION].COD_TTYPE = 1 THEN ISNULL(
                                          (SELECT TRANSACTION_SERVICES.TAX_PLANDZERO_AFF
                                           FROM TRANSACTION_SERVICES
                                                    INNER JOIN ITEMS_SERVICES_AVAILABLE
                                                               ON TRANSACTION_SERVICES.COD_ITEM_SERVICE =
               ...
               , CASE
                     WHEN [TRANSACTION].PLOTS = 1 THEN dbo.FNC_CALC_LIQ_MDR(TRANSACTION_TITLES_COST.[PERCENTAGE] +
                                                                            ISNULL(IIF([TRANSACTION].COD_TTYPE = 1,
               ...
               , dbo.FNC_CALC_LIQ_ANTICIP_DAYS
                 (
                         ...
                         ISNULL(IIF([TRANSACTION].COD_TTYPE = 1, TRANSACTION_TITLES_COST.TAX_PLANDZERO, 0), 0),
                         [TRANSACTION_TITLES].PLOT,
               ...
               , (CASE
                 ...
                 END)                                                                       AS RATE_PLAN
               , CASE
                     WHEN [TRANSACTION].COD_TTYPE = 2 AND
                          ([TRANSACTION_TITLES].TAX_PLANDZERO IS NULL OR [TRANSACTION_TITLES].TAX_PLANDZERO = 0) THEN 0
                     WHEN TRANSACTION_TITLES.IS_SPOT = 1 THEN DATEDIFF(DAY, TRANSACTION_TITLES.PREVISION_PAY_DATE,
                                                                       TRANSACTION_TITLES.ORIGINAL_RECEIVE_DATE)
                     ELSE TRANSACTION_TITLES.QTY_DAYS_ANTECIP
               ...
               , CASE
                     WHEN (SELECT COUNT(*)
                           FROM TRANSACTION_SERVICES WITH (NOLOCK)
                                    join ITEMS_SERVICES_AVAILABLE ISA on ISA.COD_ITEM_SERVICE = TRANSACTION_SERVICES.COD_ITEM_SERVICE
                           WHERE TRANSACTION_SERVICES.COD_TRAN = [TRANSACTION].COD_TRAN
...
BEGIN
    ...
    WITH TRANINFO
             AS
             (SELECT COUNT(COD_TRAN) AVAILABLE_INSTALLMENTS
                   , COD_TRAN
                   , QUOTA_TOTAL
...
BEGIN
    ...
    SELECT TOP 1 @AWAITINGSPLIT = [COD_SITUATION]
    FROM [SITUATION]
